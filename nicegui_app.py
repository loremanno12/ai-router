...
async def _poll_requests() -> None:
    ...
    try:
        if action == 'optimize':
            ...
            prompt = payload.get('prompt', '')
            # Nuovi parametri per pipeline
            preproc_mode = payload.get('preproc_mode', 'off')
            use_improver = payload.get('use_improver', False)

            # Usa pipeline se richiesto
            if preproc_mode != 'off' or use_improver:
                result = predict_with_pipeline(
                    prompt, config, model_cache,
                    preproc_mode=preproc_mode,
                    use_improver=use_improver
                )
                # Estrai routing finale
                final_routing = result.get('final_routing') or result.get('initial_routing')
                response_payload = {
                    'recommended_model': final_routing.get('recommended_model') if final_routing else None,
                    'confidence': final_routing.get('confidence') if final_routing else None,
                    'all_probabilities': final_routing.get('all_probabilities') if final_routing else None,
                    'reasons': final_routing.get('reasons') if final_routing else None,
                    'ranking': final_routing.get('ranking') if final_routing else None,
                    'routing_changed': result.get('routing_changed', False),
                    'preprocessed_prompt': result.get('preprocessed_prompt'),
                ...
                # Routing standard (senza pipeline)
                result = predict_model(prompt, config, model_cache)
                response_payload = {
                    'recommended_model': result.get('recommended_model') or result.get('predicted_model'),
...
def run(port: int = 7860) -> None:
    ...
    global predict_model, predict_with_pipeline, improve_prompt_with_ollama, check_ollama_health
    ...
    try:
        ...
        predict_with_pipeline = getattr(_predictor_mod, 'predict_with_pipeline', None)
        initialize_semantic_routing = getattr(_predictor_mod, 'initialize_semantic_routing', None)
        if metrics_collector and hasattr(_predictor_mod, 'metrics_collector'):
            _predictor_mod.metrics_collector = metrics_collector

        if initialize_semantic_routing and config:
            try:
                initialize_semantic_routing(config)
                logger.info("✓ Semantic routing initialized")
            except Exception as e:
                logger.exception("Failed to initialize semantic routing: %s", e)
    except Exception:
        predict_model = None
        predict_with_pipeline = None
