...
def train_model(config: Config, model_cache: ModelCache) -> Tuple[bool, str]:
    try:
        ...
        classifier.fit(X, y)
        # Prova a salvare i file dei modelli; in caso di PermissionError
        # tenta una directory di fallback sul filesystem temporaneo.
        try:
            ...
            logger.warning(
                "PermissionError scrivendo in %s: %s. Provo fallback in tempdir",
                config.CLASSIFIER_PATH,
                pe,
            )
            try:
                fallback_dir = Path(tempfile.gettempdir()) / "ai_router_models"
                fallback_dir.mkdir(parents=True, exist_ok=True)
                fallback_classifier = fallback_dir / config.CLASSIFIER_PATH.name
                fallback_encoder = fallback_dir / config.ENCODER_PATH.name
                with open(fallback_classifier, "wb") as f:
                    pickle.dump(classifier, f)
                with open(fallback_encoder, "wb") as f:
                    pickle.dump(label_encoder, f)
                # aggiorna i percorsi nel config per l'uso successivo
                config.CLASSIFIER_PATH = fallback_classifier
                config.ENCODER_PATH = fallback_encoder
                logger.info("Modello salvato nella directory di fallback: %s", fallback_dir)
            except Exception as e2:
                logger.exception("Impossibile salvare modello neanche nella fallback")
                return False, str(pe)
        except Exception as e:
            logger.exception("Errore scrittura modelli")
            return False, str(e)

        model_cache.set_classifier(classifier)
        model_cache.set_label_encoder(label_encoder)
        return True, f"Modello addestrato con {len(prompts)} esempi (salvato in {config.CLASSIFIER_PATH.parent})"
